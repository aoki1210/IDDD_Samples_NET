...
{
    public class カレンダーエントリークエリーサービス : AbstractQueryService
    {
        public カレンダーエントリークエリーサービス(string providerName, string connectionString)
            : base(connectionString, providerName)
        {
        }

        public カレンダーエントリデータ GetCalendarEntryDataById(string tenantId, string calendarEntryId)
        {
            return QueryObject<カレンダーエントリデータ>(
                    "select "
        ...
        public IList<カレンダーエントリデータ> GetCalendarEntryDataByCalendarId(string tenantId, string calendarId)
        {
            return QueryObjects<カレンダーエントリデータ>(
                    "select "
                    + "entry.calendar_entry_id, entry.alarm_alarm_units, entry.alarm_alarm_units_type, "
                    + "entry.calendar_id, entry.description, entry.location, "
                    + "entry.owner_email_address, entry.owner_identity, entry.owner_name, "
        ...
        public IList<カレンダーエントリデータ> GetCalendarEntriesSpanning(string tenantId, string calendarId, DateTime beginsOn, DateTime endsOn)
        {
            return QueryObjects<カレンダーエントリデータ>(
                    "select "
                    + "entry.calendar_entry_id, entry.alarm_alarm_units, entry.alarm_alarm_units_type, "
