...
{
    public class カレンダークエリーサービス : AbstractQueryService
    {
        public カレンダークエリーサービス(string providerName, string connectionString)
            : base(connectionString, providerName)
        {
        }

        public IList<カレンダーデータ> GetAllCalendarsDataByTenant(string tenantId)
        {
            return QueryObjects<カレンダーデータ>(
                    "select "
        ...
        public カレンダーデータ GetCalendarDataById(string tenantId, string calendarId)
        {
            return QueryObject<カレンダーデータ>(
                    "select "
                    + "cal.calendar_id, cal.description, cal.name, cal.owner_email_address, "
                    + "cal.owner_identity, cal.owner_name, cal.tenant_id, "
                    + "sharer.calendar_id as o_sharers_calendar_id, "
