...
{
    public class フォーラムクエリーサービス : AbstractQueryService
    {
        public フォーラムクエリーサービス(string connectionString, string providerName)
            : base(connectionString, providerName)
        {
        }

        public IList<フォーラムデータ> GetAllForumsDataByTenant(string tenantId)
        {
            return QueryObjects<フォーラムデータ>(
                    "select * from tbl_vw_forum where tenant_id = ?",
                    new JoinOn(),
                    tenantId);
        }

        public フォーラムデータ GetForumDataById(string tenantId, string forumId)
        {
            return QueryObject<フォーラムデータ>(
        ...
        public フォーラムディスカッションデータ GetForumDiscussionsDataById(string tenantId, string forumId)
        {
            return QueryObject<フォーラムディスカッションデータ>(
                    "select "
