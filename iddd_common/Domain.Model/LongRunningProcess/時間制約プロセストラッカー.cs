...
namespace SaaSOvation.Common.Domain.Model.LongRunningProcess
{
    ...
    public class 時間制約プロセストラッカー
    {
        public 時間制約プロセストラッカー(
                string tenantId,
                プロセスId processId,
                string description,
        ...
        public プロセスId ProcessId { get; private set; }
        ...
        public void InformProcessTimedOut()
        {
            ...
            {
                プロセスタイムアウト processTimedOut = null;
        ...
        public override bool Equals(object anotherObject)
        {
            ...
            {
                時間制約プロセストラッカー typedObject = (時間制約プロセストラッカー)anotherObject;
                equalObjects =
        ...
        プロセスタイムアウト ProcessTimedOutEvent()
        {
            try
            {
                var type = Type.GetType(this.ProcessTimedOutEventType);
                return (プロセスタイムアウト)Activator.CreateInstance(type, new [] { this.ProcessId });
            }
            catch (Exception e)
            {
        ...
        プロセスタイムアウト ProcessTimedOutEventWithRetries()
        {
            try
            {
                var type = Type.GetType(this.ProcessTimedOutEventType);

                return (プロセスタイムアウト)Activator.CreateInstance(type, new object[] { this.ProcessId, this.TotalRetriesPermitted, this.RetryCount } );
            }
            catch (Exception e)
            {
                throw new InvalidOperationException(
